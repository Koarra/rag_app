...
|--------|-------------|----------|---------|----------|-----------|
| **Entity Recall** | % of flagged entities detected | ≥85% | ≥80% | HIGHEST | Missing entities means undetected criminals. This is the most critical failure mode as it directly impacts public safety and regulatory compliance. **85% chosen** because: (1) baseline model achieves 87-92% recall, (2) allows for minor LLM variability while catching real degradation, (3) balances strictness with operational feasibility—100% is unrealistic for probabilistic systems. |
| **Missed Entities** | Count of undetected entities | 0 | ≤2 | HIGHEST | Absolute count provides immediate visibility into failures. Even a single missed entity may represent significant risk depending on severity of crimes involved. **Warning at ≤2** because: (1) ideal is zero misses but allows for 1-2 edge cases without immediate alarm, (2) statistically, with 85% recall on ~20-30 entities, 2-3 misses are expected, (3) provides graduated response—investigate but not emergency. **Critical at 0** means any week with zero misses is perfect performance. |
| **Crime Recall** | % of actual crimes detected per entity (averaged across all detected entities) | ≥85% | ≥80% | HIGH | Detecting an entity but missing their crimes leads to incomplete investigations, wrong prioritization, and potential compliance gaps. Critical crimes must not be missed. **85% chosen** to match entity-level recall, ensuring consistent quality standards across both detection levels. Slightly lower priority than entity detection because at least the entity is flagged for investigation. |
| **Jaccard Similarity** | Crime label set overlap - measures both missing crimes AND extra incorrect crimes (averaged per entity) | ≥0.85 | ≥0.75 | HIGH | Provides overall quality measure. Low Jaccard indicates either missing many crimes OR adding many false crimes. Complements recall by catching false positive issues. **0.85 chosen** to align with 85% recall expectation—if recall is 85% with minimal false positives, Jaccard ≈ 0.85. **0.75 warning** allows for some false positives (e.g., 85% recall + 15% extra crimes → Jaccard ≈ 0.74). |
| **Critical Crime Miss** | Count of high-severity crimes missed (fraud, money laundering, terrorism financing) | 0 | - | CRITICAL | Zero tolerance for missing critical crimes regardless of overall recall. These crimes have highest regulatory/business impact and require immediate investigation priority. **Zero tolerance** because: (1) missing fraud/money laundering triggers regulatory violations, (2) financial/reputational damage far exceeds investigation cost, (3) these crimes are specifically monitored by compliance teams. No warning threshold—any miss is critical. |
| **Entity Precision** | % of flagged entities that are actually correct | Monitor | ≥70% | LOW | While false positives consume investigation resources, they don't pose safety/compliance risk. Monitored to prevent alert fatigue but not primary concern. **70% threshold** means up to 30% false positives acceptable—better to over-flag than under-flag given asymmetric risk. Only monitored to catch systematic issues (e.g., model flagging everyone). |
